...
(async function () {
    ...
    const assetList = await getFileListRecursively(Path.join(__dirname, 'src/assets/icons'));
    ...
    await Fs.promises.writeFile(`${TS_FILE_DIR}/${TS_FILE_NAME}.d.ts`, '');
    ...
    for await (const ext of INCLUDE_EXT) {
        ...
        namespaceString = ext.charAt(0).toUpperCase() + ext.slice(1);
        await Fs.promises.writeFile(
            `${TS_FILE_DIR}/${ext}.ts`, 
            `/// <reference path="${TS_FILE_NAME}.d.ts"/>\n\n${importList[ext].join('\n')}\n\ntype ${namespaceString} = { \n${exportList[ext].map((item)=> `\n${item}: typeof ${item};`).join('\n') }\n};\nexport type { ${namespaceString} }`
        );
        await Fs.promises.appendFile(
